...
function render(){
  ...
  el('activationNote').textContent=dirty?'احفظ حدود البحث المعدلة قبل التفعيل أو بدء جولة جديدة.':state.verified?'تم التحقق من الوصول إلى Sol 5.6. يعرض السجل أدناه نتائج البحث الفعلية وأخطاءه.':'يتطلب تفعيل الجدول مفتاحًا مضبوطًا واختبار اتصال ناجحًا.';
  ...
  el('priorityNote').textContent=(state.priority_governorates||[]).length?'أولوية البحث: '+state.priority_governorates.map(g=>g.name).join('، ')+' — نحو '+number(Math.round(state.priority_share*100))+'٪ من الطلبات، والباقي لبقية المحافظات. تتناوب المنصات والمواقع داخل كل محافظة.':'';
  ...
  el('jobHistory').innerHTML=state.jobs.length?state.jobs.map(job=>`<article class="history-item"><div class="history-heading"><strong>${esc(job.target_name)} · ${esc(platforms[job.platform]||job.platform)}</strong><span>${esc(states[job.state]||job.state)}</span></div><p>${job.model?esc(job.model)+' · ':''}${job.started_at?esc(date(job.started_at))+' · ':''}${number(job.published_count)} نُشر · ${number(job.review_count)} للمراجعة · ${number(job.duplicate_count)} مكرر</p>${job.error_message?`<p class="error-detail">${esc(job.error_message)}</p>`:''}</article>`).join(''):'<p class="empty-state">لا توجد عمليات بحث مسجلة بعد. لا يعني ذلك عدم وجود عروض عقارية.</p>';
...
el('verifyConnection').onclick=()=>action('جاري اختبار اتصال Sol 5.6…',()=>request(api+'/verify',{method:'POST'}),'تم التحقق من الوصول إلى Sol 5.6. يمكنك تفعيل الجدول.');
