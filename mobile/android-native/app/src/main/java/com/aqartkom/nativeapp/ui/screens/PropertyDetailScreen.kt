    ...
    androidx.compose.foundation.lazy.LazyColumn(modifier.fillMaxSize(), contentPadding = PaddingValues(bottom = 24.dp)) {
        ...
        item {
            Column(Modifier.padding(18.dp)) {
                ...
                ElevatedCard(shape = RoundedCornerShape(20.dp)) { Row(Modifier.fillMaxWidth().padding(16.dp), verticalAlignment = Alignment.CenterVertically) { Surface(shape = RoundedCornerShape(50), color = MaterialTheme.colorScheme.primary) { Text((p.owner?.name ?: "ع").take(1), Modifier.padding(15.dp), color = Color.White, fontWeight = FontWeight.Black) }; Spacer(Modifier.width(12.dp)); Column(Modifier.weight(1f)) { Text(p.owner?.name ?: "معلن عقاري", fontWeight = FontWeight.Bold); Text(if (p.owner?.role == "agent") "مكتب عقاري" else "معلن العقار", color = MaterialTheme.colorScheme.onSurfaceVariant) } } }
                Spacer(Modifier.height(18.dp))
