...
(async()=>{
 ...
 try{
  ...
  const video=(await request('/api/me/properties/'+id+'/videos',{method:'POST',cookie:alice.cookie,form:videoForm,expected:201})).value.data[0];uploaded.push(root+video.url);
  const mediaDetail=(await request('/api/properties/'+id)).value.data;assert.equal(mediaDetail.images.length,1);assert.equal(mediaDetail.videos.length,1);assert.equal(mediaDetail.videos[0].is_primary,true);
