...
function worker() {
  ...
  const context = {URL, location: {origin: 'https://aqartkom.test'}, fetch: async () => { throw Error('offline'); },
    caches: {keys: async () => ['aqartkom-v60-shell-v1', 'aqartkom-v92-public-v1', 'other-app'], delete: async k => deleted.push(k), match: async k => ({cached: k}), open: async () => ({addAll: async list => cached.push(...list)})},
...
test('offline cache excludes personal pages and removes only app-owned older caches', async () => {
  ...
  assert.deepEqual(w.cached, ['/offline.html', '/icons/icon-192.png', '/icons/icon-512.png']);
  w.handlers.activate({waitUntil: p => work = p}); await work;
  assert.deepEqual(w.deleted, ['aqartkom-v60-shell-v1']);
});
