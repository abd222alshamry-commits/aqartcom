...
test('hotel administration works without an office; partner scope and login remain enforced',async t=>{
 ...
 const getCurrentUser=async req=>req.headers['x-test-role']==='admin'?{id:admin,role:'admin'}:req.headers['x-test-role']==='agent'?{id:agent,role:'agent'}:req.headers['x-test-role']==='user'?{id:agent,role:'user'}:null;
 ...
 new Function('app','pool','getCurrentUser','getOfficeForUser',middleware+access+routes.join('\n')+"\napp.get('/api/office/other',requireOfficeMember,(req,res)=>res.json({ok:true}));")(app,db,getCurrentUser,getOfficeForUser);
