...
app.get('/api/hotels', async(req,res)=>{try{
  ...
  const vals=[]; const where=[`h.status='active'`];
  if(req.query.includeDemo!=='true')where.push(`NOT (h.slug LIKE 'aqartkom-demo-hotel-v1-%' AND h.name LIKE 'تجريبي —%')`);
  if(req.query.region==='mashta'){vals.push('%مشتى الحلو%');where.push(`(h.city ILIKE $${vals.length} OR COALESCE(h.district,'') ILIKE $${vals.length})`);}
  if(req.query.region&&req.query.region!=='mashta')return res.status(400).json({error:'المنطقة غير صحيحة'});
  if((ci||co)&&!dateValid(ci,co))return res.status(400).json({error:'اختر تاريخ مغادرة بعد تاريخ الوصول'});
   if(req.query.lodging_type){if(!require('./host-portal').types.includes(req.query.lodging_type))return res.status(400).json({error:'نوع المنشأة غير صحيح'});vals.push(req.query.lodging_type);where.push(`h.lodging_type=$${vals.length}`);} if(city){vals.push(city);where.push(`h.city=$${vals.length}`);} if(q){vals.push(`%${q}%`);where.push(`(h.name ILIKE $${vals.length} OR h.city ILIKE $${vals.length} OR COALESCE(h.district,'') ILIKE $${vals.length})`);}
...
app.get('/api/me/dashboard', requireAuth, async (req,res)=>{
  try {
    const [props, stats, inquiries] = await Promise.all([
      pool.query(`SELECT p.*, COALESCE((SELECT json_agg(json_build_object('id',pi.id,'url',pi.url,'sort_order',pi.sort_order) ORDER BY pi.sort_order,pi.id) FROM property_images pi WHERE pi.property_id=p.id),'[]') AS images, COALESCE((SELECT json_agg(json_build_object('id',pv.id,'url',pv.url,'poster_url',pv.poster_url,'title',pv.title,'source_type',pv.source_type,'is_primary',pv.is_primary) ORDER BY pv.created_at DESC) FROM property_videos pv WHERE pv.property_id=p.id),'[]') AS videos, (SELECT COUNT(*) FROM inquiries i WHERE i.property_id=p.id AND i.status='new') AS new_inquiries FROM properties p WHERE p.owner_id=$1 AND p.deleted_at IS NULL ORDER BY p.created_at DESC`,[req.user.id]),
      pool.query(`SELECT COUNT(*) AS properties, COALESCE(SUM(p.views_count),0) AS views, (SELECT COUNT(*) FROM inquiries i JOIN properties pp ON pp.id=i.property_id WHERE pp.owner_id=$1) AS inquiries FROM properties p WHERE p.owner_id=$1 AND p.deleted_at IS NULL`,[req.user.id]),
...
app.get('/api/me/properties', requireAuth, async (req, res) => {
  const result = await pool.query('SELECT * FROM properties WHERE owner_id=$1 AND deleted_at IS NULL ORDER BY created_at DESC', [req.user.id]);
...
app.get('/api/admin/properties', requireAdmin, async (req,res)=>{ const q=String(req.query.q||'').trim(); const status=String(req.query.status||'').trim(); const vals=[]; const where=["p.deleted_at IS NULL"]; if(q){vals.push(`%${q}%`);where.push(`(p.title ILIKE $${vals.length} OR p.city ILIKE $${vals.length} OR p.district ILIKE $${vals.length})`)} if(status){vals.push(status);where.push(`p.status=$${vals.length}`)} const r=await pool.query(`SELECT p.*,u.name owner_name,u.phone owner_phone,(SELECT COUNT(*) FROM inquiries i WHERE i.property_id=p.id)::int inquiry_count FROM properties p LEFT JOIN users u ON u.id=p.owner_id ${where.length?'WHERE '+where.join(' AND '):''} ORDER BY p.created_at DESC LIMIT 300`,vals);res.json({data:r.rows}); });
...
app.get('/api/me/recommendations',requireAuth,async(req,res)=>{try{res.json(await recommendationsFor(req.user.id,Math.max(1,Math.min(30,Math.floor(Number(req.query.limit)||12)))))}catch(e){console.error(e);res.status(500).json({error:'تعذر تحميل العقارات المقترحة'})}});
