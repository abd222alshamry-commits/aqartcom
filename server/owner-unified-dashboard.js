module.exports=function(app,{pool,requireAdmin}){
 ...
 app.get('/api/admin/owner-unified-dashboard',requireAdmin,async(req,res)=>{try{
  ...
  const [props,leads,deals,finance,hotels,offices,ads,risks,proposals]=await Promise.all([
   ...
   safe(`SELECT COALESCE(SUM(total_amount),0) billed,COALESCE(SUM(paid_amount),0) collected,COALESCE(SUM(total_amount-paid_amount) FILTER(WHERE status NOT IN ('paid','void')),0) outstanding FROM electronic_invoices WHERE issue_date>=CURRENT_DATE-$1`,[days]),
   ...
   safe(`SELECT platform,COALESCE(SUM(spend),0) spend,COALESCE(SUM(conversions),0) conversions,COALESCE(SUM(conversion_value),0) conversion_value FROM ad_platform_daily_metrics WHERE metric_date>=CURRENT_DATE-$1 GROUP BY platform ORDER BY spend DESC`,[days]),
