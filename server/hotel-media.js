...
function register(app,{pool,requireOfficeMember,ownedHotel,uploadDir,createVideoPoster,mediaStore=require('./media-storage').createMediaStorage({uploadDir})}){
 ...
 for(const base of ['/api/office/hotels/:id','/api/office/hotels/:id/rooms/:roomId']){
  for(const kind of ['images','videos']){
   ...
   app.post(base+'/'+kind,requireOfficeMember,scope,mediaStore.uploadGate,(req,res,next)=>uploader(req,res,e=>e?res.status(400).json({error:'تعذر الرفع: تأكد من الصيغة والعدد وحجم الملف'}):next()),async(req,res)=>{
    req.mediaProcessing=true;let client,transaction=false;const batch=mediaStore.batch();const cleanup=(req.files||[]).map(f=>f.path);
    try{
     if(!req.files?.length)return res.status(400).json({error:'اختر ملفات للرفع'});
     const items=[];
     for(const f of req.files){const handle=await fs.open(f.path,'r');const head=Buffer.alloc(12);try{await handle.read(head,0,12,0);}finally{await handle.close();}if(!signature(head,kind))throw Error('محتوى الملف لا يطابق صورة أو فيديو مدعوم');const item={url:'/uploads/'+f.filename};if(kind==='videos'){item.poster_url=await createVideoPoster(f.path);if(!item.poster_url)throw Error('تعذر قراءة الفيديو؛ اختر ملف فيديو صالحًا');cleanup.push(path.join(uploadDir,path.basename(item.poster_url)));}if(item.poster_url)item.poster_url=await batch.add(path.join(uploadDir,path.basename(item.poster_url)));item.url=await batch.add(f.path);items.push(item);}
     ...
     const value=[...before,...items];await client.query(`UPDATE ${req.mediaTable} SET ${kind}=$1::jsonb,updated_at=NOW() WHERE id=$2`,[JSON.stringify(value),req.mediaId]);await client.query('COMMIT');transaction=false;await batch.commit();
     res.status(201).json({data:items});
    }catch(e){if(transaction)await client.query('ROLLBACK').catch(()=>{});await batch.rollback();await Promise.all(cleanup.map(p=>fs.unlink(p).catch(()=>{})));res.status(400).json({error:e.message==='محتوى الملف لا يطابق صورة أو فيديو مدعوم'||e.message.startsWith('الحد الأقصى')||e.message.startsWith('تعذر قراءة الفيديو')?e.message:'تعذر حفظ الوسائط'});}finally{client?.release();req.releaseMediaUpload?.();}
   });
  }
