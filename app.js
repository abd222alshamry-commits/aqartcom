...
function updateAuthUI(){window.ListingActions?.refreshPermissions();const btn=document.getElementById('login');if(currentUser){btn.textContent=currentUser.name;btn.onclick=openUserAccount}else{btn.textContent='دخول / تسجيل';btn.onclick=openAuth}}
...
function openUserAccount(){const m=modalHtml('accountModal',`<h2>حسابي</h2><p>مرحباً ${escapeHtml(currentUser.name)}</p>`,`<div class="account-card"><b>${escapeHtml(currentUser.email)}</b><span>${escapeHtml(currentUser.phone||'لا يوجد رقم هاتف')}</span><small>نوع الحساب: ${currentUser.role==='admin'?'مدير':currentUser.role==='agent'?'مكتب عقاري':'مستخدم'}</small></div><a class="listing-manage-link" href="/my-listings.html">إدارة الإعلانات — تعديل وحذف ومشاركة</a><button id="myProps">عقاراتي</button><button id="savedSearchesBtn">🔔 عمليات البحث المحفوظة</button><button id="notificationsBtn">🔔 التنبيهات</button><button id="logoutBtn">تسجيل الخروج</button>${currentUser.role==='admin'?'<button id="adminBtn">لوحة الإدارة</button>'+(!Array.isArray(currentUser.admin_permissions)||currentUser.admin_permissions.includes('offers.read')?'<button id="reviewOffersBtn">مراجعة العروض وعروض المكاتب</button>':''):''}`);if(m.querySelector('#reviewOffersBtn'))m.querySelector('#reviewOffersBtn').onclick=()=>location.href='/offer-review.html';if(m.querySelector('#adminBtn'))m.querySelector('#adminBtn').onclick=()=>location.href='/admin.html';m.querySelector('#savedSearchesBtn').onclick=()=>{m.remove();openSavedSearches()};m.querySelector('#notificationsBtn').onclick=()=>{m.remove();openNotifications()};m.querySelector('#logoutBtn').onclick=async()=>{await fetch(`${API}/auth/logout`,{method:'POST',credentials:'same-origin'});currentUser=null;favorites.clear();updateAuthUI();m.remove();document.querySelector('#fav span').textContent='0';showToast('تم تسجيل الخروج')};m.querySelector('#myProps').onclick=()=>{m.remove();openAccount()}}

function clearMapSearch(){
 geoSearch.lat=null;geoSearch.lng=null;geoSearch.polygon=[];geoSearch.drawing=false;
 for(const key of ['marker','polygonLayer']){if(geoSearch[key])geoSearch.map?.removeLayer(geoSearch[key]);geoSearch[key]=null;}
 document.getElementById('commuteMinutes').value='';document.getElementById('drawArea')?.classList.remove('active');updateGeoNote();
}
function selectedPriorityRegion(){const city=document.getElementById('city').value,town=document.getElementById('district').value.trim();return city==='طرطوس'&&town==='مشتى الحلو'?'mashta':city==='دمشق'&&!town?'damascus':'';}
function syncPriorityRegion(){
 const selected=selectedPriorityRegion();document.querySelectorAll('[data-priority-region]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.priorityRegion===selected)));
 const url=new URL(location.href);selected?url.searchParams.set('region',selected):url.searchParams.delete('region');history.replaceState(history.state,'',url);
}
function selectPriorityRegion(region){
 const target=region==='mashta'?['طرطوس','مشتى الحلو']:['دمشق',''];const city=document.getElementById('city');city.value=target[0];city.dispatchEvent(new Event('change'));document.getElementById('district').value=target[1];clearMapSearch();syncPriorityRegion();
}
const initialRegion=new URLSearchParams(location.search).get('region');
if(['mashta','damascus'].includes(initialRegion))selectPriorityRegion(initialRegion);
document.querySelectorAll('[data-priority-region]').forEach(button=>button.addEventListener('click',async()=>{selectPriorityRegion(button.dataset.priorityRegion);await refreshGeoSearch();document.getElementById('listings').scrollIntoView({behavior:'smooth'});}));
['city','district'].forEach(id=>document.getElementById(id).addEventListener('change',syncPriorityRegion));
// Search / navigation
document.getElementById('saveSearchBtn')?.addEventListener('click',saveCurrentSearch);if('serviceWorker' in navigator)navigator.serviceWorker.register('/service-worker.js').catch(()=>{});refreshGeoSearch();me();
...
(() => {
  ...
  const close = () => {setOpen(false);navigation.querySelectorAll('details').forEach(menu=>menu.open=false);};
  ...
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape' && (button.getAttribute('aria-expanded') === 'true'||navigation.querySelector('details[open]'))) {
      close();
      button.focus();
    }
  });
