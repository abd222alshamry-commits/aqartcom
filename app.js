...
function openUserAccount(){const m=modalHtml('accountModal',`<h2>حسابي</h2><p>مرحباً ${escapeHtml(currentUser.name)}</p>`,`<div class="account-card"><b>${escapeHtml(currentUser.email)}</b><span>${escapeHtml(currentUser.phone||'لا يوجد رقم هاتف')}</span><small>نوع الحساب: ${currentUser.role==='admin'?'مدير':currentUser.role==='agent'?'مكتب عقاري':'مستخدم'}</small></div><button id="myProps">عقاراتي</button><button id="savedSearchesBtn">🔔 عمليات البحث المحفوظة</button><button id="notificationsBtn">🔔 التنبيهات</button><button id="logoutBtn">تسجيل الخروج</button>${currentUser.role==='admin'?'<button id="adminBtn">لوحة الإدارة</button>':''}`);if(m.querySelector('#adminBtn'))m.querySelector('#adminBtn').onclick=()=>location.href='/admin.html';m.querySelector('#savedSearchesBtn').onclick=()=>{m.remove();openSavedSearches()};m.querySelector('#notificationsBtn').onclick=()=>{m.remove();openNotifications()};m.querySelector('#logoutBtn').onclick=async()=>{await fetch(`${API}/auth/logout`,{method:'POST',credentials:'same-origin'});currentUser=null;favorites.clear();updateAuthUI();m.remove();document.querySelector('#fav span').textContent='0';showToast('تم تسجيل الخروج')};m.querySelector('#myProps').onclick=()=>{m.remove();openAccount()}}
...
let assistantSession=null;const assistantPanel=document.getElementById('assistantPanel'),assistantMessages=document.getElementById('assistantMessages'),assistantResults=document.getElementById('assistantResults');
function assistantMsg(text,role='bot'){const d=document.createElement('div');d.className='assistant-msg '+role;d.textContent=text;assistantMessages.appendChild(d);assistantMessages.scrollTop=assistantMessages.scrollHeight}
function assistantRenderResults(rows=[]){assistantResults.innerHTML=rows.map(p=>`<div class="assistant-result" data-assistant-property="${p.id}"><div><b>${escapeHtml(p.title)}</b><small>${escapeHtml(p.city)} ${p.district?'• '+escapeHtml(p.district):''} • ${p.rooms||'—'} غرف • ${p.area||'—'} م²</small></div><strong>${money(p.price,p.currency||'USD')}</strong></div>`).join('');assistantResults.querySelectorAll('[data-assistant-property]').forEach(x=>x.onclick=()=>location.href='/property.html?id='+encodeURIComponent(x.dataset.assistantProperty))}
document.getElementById('assistantFab')?.addEventListener('click',()=>assistantPanel.classList.toggle('open'));document.getElementById('assistantClose')?.addEventListener('click',()=>assistantPanel.classList.remove('open'));document.getElementById('assistantReset')?.addEventListener('click',()=>{assistantSession=null;assistantResults.innerHTML='';assistantMessages.innerHTML='';assistantMsg('ابدأ بحثاً جديداً. مثال: أريد فيلا للإيجار في دمشق بحدود 1500 دولار.')});document.getElementById('assistantForm')?.addEventListener('submit',async e=>{e.preventDefault();const input=document.getElementById('assistantInput'),message=input.value.trim();if(!message)return;assistantMsg(message,'user');input.value='';input.disabled=true;try{const r=await fetch(`${API}/chatgpt`,{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({message})});const j=await r.json();if(!r.ok)throw Error(j.error||'تعذر البحث');assistantSession=j.session_id||assistantSession;assistantMsg(j.reply);if(j.results)assistantRenderResults(j.results||[])}catch(err){assistantMsg(err.message)}finally{input.disabled=false;input.focus()}});
